...
const turn_indicator = document.getElementById("game-turn");
const result_indicator = document.getElementById("game-result");
let player_turn = true;
let game_over = false;
let player_won = false;
let enemy_won = false;
function setButton(button, is_player) {
    ...
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            if (game_over) {
                ...
                const available_buttons = Array.from(buttons.entries()).filter(([_button_id, button]) => {
                ...
                if (available_buttons.length === 0) {
                    ...
                    const random_button = available_buttons[Math.floor(Math.random() * available_buttons.length)];
...
function checkForVictory(buttons) {
    return new Promise((resolve, reject) => {
        const grid = Array.from(buttons.values());
        // Check - rows
        for (let y = 0; y < 3; ++y) {
